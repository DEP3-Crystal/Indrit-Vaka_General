...
public class VendingMachine {
    public static void main(String[] args) {
        ...
        charge = vendingMachine.getChange(inputtedMoney, pCost);
     ...
     * */
    public int[] getChange(double inputtedMoney, double productCost) {
        int[] change = new int[6];
        //First we convert all money in cent, so we can use %
        //Second we get the total money we have to return
        if (inputtedMoney < 0 || productCost < 0) {
            throw new IllegalArgumentException("The inputted money or product cost can't be negative");
        }
        int totalChange = (int) (inputtedMoney * 100 - productCost * 100);
        int[] money = new int[]{1, 5, 10, 25, 50, 100};
